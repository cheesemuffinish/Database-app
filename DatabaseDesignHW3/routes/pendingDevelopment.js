...
router.post('/', function(req, res, next) {
  ...
  connection.on('connect', function(err) {
    if (err) {
      ...
      var PendingDevelopmentItemID;
      var PendingDevelopmentItemStatus;


      // Populate form name variables
      for (var i = 0; i < req.body.formData.length; i++){
        if (req.body.formData[i].name == "PendingDevelopmentItemID") {
          PendingDevelopmentItemID = req.body.formData[i].value;
        } else if (req.body.formData[i].name == "PendingDevelopmentItemStatus") {
          PendingDevelopmentItemStatus = req.body.formData[i].value
        }
      }

      var queryStatement;
      queryStatement = `INSERT INTO work.status_changes (
          item_id,
          previous_status,
          new_status,
          added_by
         ...
         VALUES (
          '${PendingDevelopmentItemID}',
          'Pending Development',
          '${PendingDevelopmentItemStatus}',
          'Joe-PC\\Joe'
        );
        update work.items set status = '${PendingDevelopmentItemStatus}'
        where id = '${PendingDevelopmentItemID}'`;
